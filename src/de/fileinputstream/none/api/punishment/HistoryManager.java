...
public class HistoryManager {
    ...
    public static void addHistoryEntry(String Typ, String UUID, String Grund, String von, String Dauer, String Name) {
        ...
        Bootstrap.getMysql().update("INSERT INTO History (Typ, UUID, Grund, Von, Dauer, Datum, Name) VALUES ('" + Typ + "','" + UUID + "','" + Grund + "','" + von + "','" + Dauer + "','" + time.format(currentTime) + "','" + Name + "')");
    }

    public static void delHistory(String UUID) {
        Bootstrap.getMysql().update("DELETE FROM History WHERE UUID='" + UUID + "'");
    }

    public static boolean isRegistered(String UUID) {
        ResultSet rs = Bootstrap.getMysql().query("SELECT UUID FROM History WHERE UUID='" + UUID + "'");
        try {
            return rs.next();
    ...
    public static String getReason(String UUID) {
        ResultSet rs = Bootstrap.getMysql().query("SELECT * FROM History WHERE UUID='" + UUID + "'");
        try {
            if (rs.next()) {
    ...
    public static String getDauer(String UUID) {
        ResultSet rs = Bootstrap.getMysql().query("SELECT Dauer FROM History WHERE UUID='" + UUID + "'");
        try {
            if (rs.next()) {
