...
import de.fileinputstream.mytraz.api.time.MuteUnit;
            ...
            if ((sender instanceof ConsoleCommandSender)) {
                if (args.length >= 4) {
                    ...
                    if (Bootstrap.getInstance().getMysql().isConnected()) {
                        ...
                        if (Bootstrap.getInstance().getMuteManager().isMuted(UUIDFetcher.getUUID(playername).toString())) {
                        ...
                        if (unitList.contains(unit.toLowerCase())) {
                            ...
                            Bootstrap.getInstance().getMuteManager().mute(UUIDFetcher.getUUID(playername).toString(), Player, reason, seconds, Dauer, "CONSOLE");
                            Bootstrap.getInstance().getHistoryManager().addHistoryEntry("TempMute", UUIDFetcher.getUUID(playername).toString(), reason, "CONSOLE", Dauer, Player);
        ...
        if (p.hasPermission("api.ban.tempmute")) {
            if (args.length >= 4) {
                ...
                if (Bootstrap.getInstance().getMysql().isConnected()) {
                    ...
                    if (!rank.equalsIgnoreCase("admin") && RankManager.getRank(UUIDFetcher.getUUID(playername2).toString()).equalsIgnoreCase("admin")) {
                    ...
                    if (Bootstrap.getInstance().getMuteManager().isMuted(UUIDFetcher.getUUID(playername2).toString())) {
                    ...
                    if (unitList2.contains(unit2.toLowerCase())) {
                        ...
                        Bootstrap.getInstance().getMuteManager().mute(UUIDFetcher.getUUID(playername2).toString(), playername2, reason2, seconds2, Dauer2, p.getName());
                        Bootstrap.getInstance().getHistoryManager().addHistoryEntry("TempMute", UUIDFetcher.getUUID(playername2).toString(), reason2, p.getName(), Dauer2, Player2);
