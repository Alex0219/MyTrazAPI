        ...
        if ((sender instanceof Player)) {
            ...
            if (p.hasPermission("api.ban.unmute")) {
                if (args.length == 1) {
                    ...
                    if (!Bootstrap.getInstance().getMuteManager().isMuted(getUUID(Player))) {
                        StringBuilder sb = new StringBuilder();

                        sender.sendMessage("§cSystem §7● §cDieser Spieler ist derzeit nicht gemutet.");
                        return true;
                    }
                    Bootstrap.getInstance().getMuteManager().unmute(getUUID(Player));
            ...
            Bootstrap.getInstance().getMuteManager().unmute(getUUID(Player2));
