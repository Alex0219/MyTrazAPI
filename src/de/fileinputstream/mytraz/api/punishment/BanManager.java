...
public class BanManager {
    ...
    public static String getBanID(String UUID) {
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT * FROM Bans WHERE UUID='" + UUID + "'");
        try {
            if (rs.next()) {
    ...
    public void ban(String UUID, String Playername, String Reason, long seconds, String Dauer, String operator, String BanID) {
        long end = 0L;
        ...
        Bootstrap.getInstance().getMysql().update("INSERT INTO Bans (Spielername, UUID, Ende, Grund, Dauer, Banner, BanID) VALUES ('" + Player + "','" + UUID + "','" + end + "','" + Reason + "','" + Dauer + "','" + operator + "','" + BanID + "')");
    ...
    public void unban(String UUID) {
        Bootstrap.getInstance().getMysql().update("DELETE FROM Bans WHERE UUID='" + UUID + "'");
    }

    public boolean isBanned(String UUID) {
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT Ende FROM Bans WHERE UUID='" + UUID + "'");
        try {
            return rs.next();
    ...
    public String getDauer(String UUID) {
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT Dauer FROM Bans WHERE UUID='" + UUID + "'");
        try {
            if (rs.next()) {
    ...
    public List<String> getBannedPlayers() {
        ...
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT * FROM Bans");
        try {
            while (rs.next()) {
    ...
    public String createbanID(String chars, int length) {
    ...
    public String getReasonFromBanID(String banid) {
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT * FROM Bans WHERE BanID='" + banid + "'");
        try {
            if (rs.next()) {
                return rs.getString("Grund");
            }
    ...
    public String getDauerFromBanID(String banid) {
        ResultSet rs = Bootstrap.getInstance().getMysql().query("SELECT * FROM Bans WHERE BanID='" + banid + "'");
        try {
            if (rs.next()) {
                return rs.getString("Dauer");
