...
public class CommandTPAAccept implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String string, String[] args) {
        if (commandSender instanceof Player) {
            ...
            if (args.length == 0)
                player.sendMessage("§bMC-Survival.de §7» §7Bitte verwende /tpaccept <Name>");
                ...
                if (!senders.contains(targetPlayer.getUniqueId())) {
                    player.sendMessage("§bMC-Survival.de §7» §3" + targetPlayer.getName()
                            + " §7hat dir keine §3TPA-Anfrage §7gesendet");
                    return false;
                }
                ...
                player.sendMessage("§bMC-Survival.de §7» §7Du hast die §3TPA-Anfrage §7von §3" + targetPlayer.getName() + " §7angenommen.");
                targetPlayer.sendMessage("§bMC-Survival.de §7» §3" + player.getName() + " §7hat deine §3TPA-Anfrage §7angenommen.");
